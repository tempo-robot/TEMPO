    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, num_frames: int = 1):
        ...
        Args:
            pixel_values (`torch.FloatTensor]` of shape `(batch_size, channels, height, width)`)
               The tensors corresponding to the input images. For TEMPO-MOT's video encoder, this is
               `(batch_size * num_frames, channels, height, width)` with frames folded into the
               batch dimension; only the current frame's tokens are returned.
            num_frames (`int`, *optional*, defaults to 1):
               Number of stacked video frames per image (TEMPO-MOT visual memory).
        Returns:
