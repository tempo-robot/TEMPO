...
class VLASHDataset(LeRobotDataset):
    ...
    def __init__(
        ...
        camera_brightness: dict[str, float] | None = None,
    ):
        ...
        Args:
            ...
            camera_brightness: Optional per-camera multiplicative brightness
                applied after decoding (frames are float in [0, 1], clipped
                after scaling).
        """
        self.max_delay_steps = max_delay_steps
        self.camera_brightness: dict[str, float] = dict(camera_brightness or {})
    ...
    def _apply_camera_brightness(self, item: dict) -> None:
        if not self.camera_brightness:
            return
        for cam, scale in self.camera_brightness.items():
            if cam in item and scale != 1.0:
                item[cam] = (item[cam] * scale).clamp_(0.0, 1.0)
    ...
    def __getitem__(self, idx) -> dict:
        ...
        self._apply_camera_brightness(item)
...
class SharedObservationVLASHDataset(VLASHDataset):
    ...
    def __getitem__(self, idx) -> dict:
        ...
        self._apply_camera_brightness(base_item)
...
def shared_observation_collate_fn(batch: list[dict]) -> dict:
    ...
    # Motion future is per-offset; motion_past, sam2_pooled, action_history are
    # observation-time (shared) and fall through to default_collate.
    motion_future_present = "observation.motion_future" in batch[0]
    if motion_future_present:
        per_offset_keys = per_offset_keys + [
            "observation.motion_future", "observation.motion_future_is_pad"
        ]
    ...
    if motion_future_present:
        motion_dim = batch[0]["observation.motion_future"].shape[-1]
        padded_motion_future = torch.zeros(
            batch_size, max_offsets, motion_dim, device=device
        )
        padded_motion_future_is_pad = torch.ones(
            batch_size, max_offsets, dtype=torch.bool, device=device
        )
        for i, item in enumerate(batch):
            n = item["num_offsets"]
            padded_motion_future[i, :n] = item["observation.motion_future"]
            padded_motion_future_is_pad[i, :n] = item["observation.motion_future_is_pad"]
        result["observation.motion_future"] = padded_motion_future  # [B, max_offsets, motion_dim]
        result["observation.motion_future_is_pad"] = padded_motion_future_is_pad  # [B, max_offsets]
